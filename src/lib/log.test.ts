...
describe('logger module', () => {
	beforeEach(() => {
		...
		vi.mocked(console.log).mockReturnThis().mockClear();
	});
	...
	it('logImportant should log text when doLogImportant is true', () => {
		const text = 'Important message';
		logger.logImportant(text);
		expect(console.log).toHaveBeenCalledWith(text);
	});

	it('logInfo should not log text when doLogInfo is false', () => {
		const text = 'Info message';
		logger.logInfo(text);
		expect(console.log).not.toHaveBeenCalled();
	});

	it('logDebug should not log text when doLogDebug is false', () => {
		const text = 'Debug message';
		logger.logDebug(text);
		expect(console.log).not.toHaveBeenCalled();
	});

	it('logInfo and logDebug should log text when setLogLevel is called with verbose level', () => {
	...
	it('setLogLevel should correctly set log levels', () => {
